    ...
    try {
      const res = await fetch('/api/credits', {
        ...
        body: JSON.stringify({
          ...
          amount: parseFloat(form.amount.replace(/\s/g, '').replace(/,/g, '.')),
          type: form.type,
        }),
      });
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-slate-500">Платежи по кредитам и лизингу (ниже EBITDA)</p>
      </div>
          ...
          <div className="bg-white rounded-xl border border-slate-200 p-6 shadow-sm">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <select
                  value={form.type}
                  onChange={(e) => setForm((f) => ({ ...f, type: e.target.value as 'CREDIT' | 'LEASING' }))}
                  className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm text-slate-900"
                >
                  <option value="CREDIT">Кредит</option>
                  <option value="LEASING">Лизинг</option>
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="Банк / лизинговая компания"
                  className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm text-slate-900"
                  required
                />
              </div>
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">Назначение</label>
                <input
                  type="text"
                  value={form.purpose}
                  ...
                  {loading ? (
                    ...
                    payments.map((p) => (
                      <tr key={p.id} className="hover:bg-slate-50">
                        ...
                        <td className="px-4 py-3 text-sm text-slate-600 max-w-xs truncate">{p.purpose || '—'}</td>
