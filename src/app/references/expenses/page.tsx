...
import { SUBDIVISIONS, LOGISTICS_STAGE_LABELS } from '@/lib/constants';
...
export default function ExpensesRefPage() {
  const [cats, setCats] = useState<Category[]>([]);
  const [loading, setLoading] = useState(true);
  const [editing, setEditing] = useState<string | null>(null);
  const [form, setForm] = useState({
    name: '',
    subdivision: 'pickup_msk',
    type: 'COGS' as string,
  });
  ...
  const bySubdivision = cats.reduce((acc, c) => {
  ...
  const getSubdivisionLabel = (key: string) => {
    const sub = SUBDIVISIONS.find((s) => s.id === key);
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-slate-900">Справочник расходов</h1>
        <p className="text-slate-500">Категории по подразделениям для ручного ввода затрат</p>
      </div>
      ...
      <form onSubmit={handleAdd} className="bg-white rounded-xl border border-slate-200 p-4 space-y-3">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          ...
          <div>
            <label className="block text-sm text-slate-600 mb-1">Подразделение</label>
            <select
        ...
        {Object.entries(bySubdivision).map(([key, items]) => (
          <div key={key} className="bg-white rounded-xl border border-slate-200 overflow-hidden">
            <div className="px-4 py-2 bg-slate-50 font-medium text-slate-700">
              {getSubdivisionLabel(key)}
            </div>
            <table className="min-w-full">
              <thead>
                <tr className="border-b border-slate-100">
                  <th className="px-4 py-2 text-left text-sm text-slate-600">Название</th>
                  <th className="px-4 py-2 text-left text-sm text-slate-600">Тип</th>
